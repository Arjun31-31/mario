...
platform=createSprite(100,850,4000,10);
platform.visible=false;

OGmario=createSprite(200,h-400,50,60);
...
function draw() {
    ...
    if(keyDown("w")){
        if(OGmario.velocityY>0){
            jumpSound.play();
            console.log(OGmario.velocityY);
        }
    ...
    for(var i =0;i<brickBorderGroup.length;i++){
        ...
        if(temp.isTouching(OGmario)){
            ...
            headBashChar.velocityX=0;
            headBashChar.velocityY=headBashChar.velocityY+1.50;
            if(headBashChar.isTouching(platform)){
                console.log("noi");
            }
            headBashChar.addImage(headBashCharIMAGE);
            headBashChar.lifetime=110;
            score++;
        }
    }
...
function enemies() {
    if(frameCount%100==0){
    var obstacle= createSprite(2000,750,12,12)
